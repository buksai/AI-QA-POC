...
/** Trade scenario 46 — healthy baseline scenario, quantity-only assertion
 * (no valuation dependency, so unaffected by fee toggle or pricing date). */
public class TradeScenario46 {
    private static final BackendClient backend = new BackendClient();

    public static void testScenario46QuantityOnly() {
        String tradeId = backend.createTrade("Copper Concentrate", "Counterparty-46");
        backend.addTranche(tradeId, "2026-09", 920.0, "Buenaventura", "Qingdao");
        backend.confirmTrade(tradeId);
        double qty = backend.getTradeTotalQuantity(tradeId);
        assertNumeric("trade.totalQuantityTonnes", 920.0, qty, 0.001);
    }
