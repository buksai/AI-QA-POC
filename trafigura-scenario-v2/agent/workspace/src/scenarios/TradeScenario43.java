...
/** Trade scenario 43 — part of the weekly regression pack.
 * Contains PATTERN-003: valuation baseline drift after fee change. */
public class TradeScenario43 {
    private static final BackendClient backend = new BackendClient();

    public static void testScenario43FeeSensitiveValuation() {
        String tradeId = backend.createTrade("Copper Concentrate", "Counterparty-43");
        backend.addTranche(tradeId, "2026-09", 2150.0, "Buenaventura", "Qingdao");
        backend.confirmTrade(tradeId);
        double val = backend.getValuationTotalUsd(tradeId);
        // Baseline assumes fee OFF: qty * 4250.00
        assertNumeric("valuation.totalValueUsd", 2150 * 4250.0, val, 0.01);
    }
