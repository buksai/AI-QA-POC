...
/** Trade scenario 48 — healthy baseline scenario, quantity-only assertion
 * (no valuation dependency, so unaffected by fee toggle or pricing date). */
public class TradeScenario48 {
    private static final BackendClient backend = new BackendClient();

    public static void testScenario48QuantityOnly() {
        String tradeId = backend.createTrade("Copper Concentrate", "Counterparty-48");
        backend.addTranche(tradeId, "2026-09", 960.0, "Buenaventura", "Qingdao");
        backend.confirmTrade(tradeId);
        double qty = backend.getTradeTotalQuantity(tradeId);
        assertNumeric("trade.totalQuantityTonnes", 960.0, qty, 0.001);
    }
